...
class SEDash extends CI_Controller {
	...
	private $tname = "tmc_pservice_langgar";
	...
	public function datatable_all()
	{
		...
		if($auth==$this->token){
			...
			$this->db->select("tgl,jam,langgar,jenis,saluran,verifikasi");
	...
	public function by_status(){
		...
		if($auth==$this->token){
			...
			$df=trim($df)==''?date('Y-m-d'):$df;
			$dt=trim($dt)==''?date('Y-m-d'):$dt;
			$this->db->select("verifikasi,count(verifikasi) as cnt");
			$this->db->from($this->tname);
			$this->db->where("tgl >=",$df);
			$this->db->where("tgl <=",$dt);
			$this->db->group_by("verifikasi");
			$data = $this->db->get()->result_array();
		}
	...
	public function by_category_perda(){
		$auth=$this->input->get_request_header('X-token', TRUE);
		$data=array();
		if($auth==$this->token){
			$df=$this->input->post('from');
